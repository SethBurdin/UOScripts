...
class Animal:
    ...
    maxTamingSkill = -1
    packType = None
    # True for creatures that spawn blue (notoriety 1) even before taming (e.g. ki-rin).
    # These require an expanded notoriety filter and different tame-success detection.
    spawnsBlue = False

    def __init__ ( self, name, mobileID, color, minTamingSkill, maxTamingSkill, packType, spawnsBlue = False ):
        self.name = name
        self.mobileID = mobileID
        self.color = color
        self.minTamingSkill = minTamingSkill
        self.maxTamingSkill = maxTamingSkill
        self.spawnsBlue = spawnsBlue
...
animals = {
    ...
    'snow leopard': Animal( 'snow leopard', 0x0040, 0x0000, 55, 80, [ 'feline' ] ),
    'snow leopard2': Animal( 'snow leopard', 0x0041, 0x0000, 55, 80, [ 'feline' ] ),
    ...
    'white wolf (dark grey)': Animal( 'white wolf', 0x0022, 0x0000, 65, 90, [ 'canine' ] ),
    'white wolf (light grey)': Animal( 'white wolf', 0x0025, 0x0000, 65, 90, [ 'canine' ] ),
    ...
    # Unicorn spawns blue (notoriety 1). Body 0x007A, color 0x0000. Skill range 85-100.
    'unicorn': Animal( 'unicorn', 0x007A, 0x0000, 85, 100, None, True ),
    ...
    'lion': None,
    # Ki-rin spawns blue (notoriety 1) - requires spawnsBlue=True so FindAnimalToTame expands
    # the notoriety filter. Body ID 0x0084 (132).
    'ki-rin': Animal( 'ki-rin', 0x0084, 0x0000, 95, 120, None, True ),
    'nightbear': None,
    'nightdragon': None,
    'nightfrenzy': None,
    'nightmare': None,
...
def GetAnimalIDsForPlayerSkill( playerTamingSkill, minimumTamingDifficulty = 0 ):
    '''
    Looks through the list of tameables for animals appropriate for the player's current skill level
    Returns animals where:
    - minTamingSkill >= minimumTamingDifficulty (floor setting)
    - Animal is within skill range: player can attempt and can gain skill
    '''
    global animals
    ...
    for animal in animals:
        if animals[ animal ] == None:
            continue
        
        # Skip if below the minimum difficulty floor
        if animals[ animal ].minTamingSkill < minimumTamingDifficulty:
            continue
        
        # Check if animal is within player's skill range
        # Allow animals up to 10 skill above the player's current level (can still attempt, just lower success rate)
        # Player skill should be >= minTamingSkill - 10 (can attempt with some chance)
        # Player skill should be <= maxTamingSkill + 10 (can still gain skill)
        if playerTamingSkill >= animals[ animal ].minTamingSkill - 10 and playerTamingSkill <= animals[ animal ].maxTamingSkill + 10:
            if not animalList.Contains( animals[ animal ].mobileID ):
                animalList.Add( animals[ animal ].mobileID )

    return animalList


def GetValidAnimalPairsForPlayerSkill( playerTamingSkill, minimumTamingDifficulty = 0 ):
    '''
    Returns a set of (mobileID, color) tuples for animals valid at the given skill level.
    Used to post-filter Mobiles.ApplyFilter results when multiple species share a body ID
    (e.g. cat 0x00C9/color-0 vs hellcat 0x00C9/color-0x647).
    '''
    global animals
    ...
    for animal in animals:
        ...
        if animals[ animal ].minTamingSkill < minimumTamingDifficulty:
            continue
        if ( playerTamingSkill >= animals[ animal ].minTamingSkill - 10 and
                playerTamingSkill <= animals[ animal ].maxTamingSkill + 10 ):
            validPairs.add( ( animals[ animal ].mobileID, animals[ animal ].color ) )
    return validPairs


def GetValidAnimalNamesForPlayerSkill( playerTamingSkill, minimumTamingDifficulty = 0 ):
    '''
    Returns a set of lowercase animal names valid at the given skill level.
    Used as a secondary post-filter for species that share both body ID and color
    (e.g. polar bear vs rideable polar bear, both 0x00D5/color-0).
    '''
    global animals
    ...
    for animal in animals:
        ...
        if ( playerTamingSkill >= animals[ animal ].minTamingSkill - 10 and
                playerTamingSkill <= animals[ animal ].maxTamingSkill + 10 ):
            validNames.add( animals[ animal ].name.lower() )
    return validNames


def GetBlueSpawningBodyIDsForPlayerSkill( playerTamingSkill, minimumTamingDifficulty = 0 ):
    '''
    Returns a Python set of body IDs for animals that naturally spawn blue (notoriety 1)
    and are valid at the given skill level. Used to expand the notoriety filter in
    FindAnimalToTame so that blue-spawning tameables (e.g. ki-rin) are included.
    '''
    global animals
    ...
    for animal in animals:
        if animals[ animal ] == None:
            continue
        if not animals[ animal ].spawnsBlue:
            continue
        if animals[ animal ].minTamingSkill < minimumTamingDifficulty:
            continue
        if ( playerTamingSkill >= animals[ animal ].minTamingSkill - 10 and
                playerTamingSkill <= animals[ animal ].maxTamingSkill + 10 ):
            bodyIDs.add( animals[ animal ].mobileID )
    return bodyIDs

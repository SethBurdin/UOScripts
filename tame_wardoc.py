'''
Description: Patrols a Wardoc spawn area, roaming between waypoints looking
    for Wardoc mobs to tame.  When follower slots are full, directs pets to
    kill a nearby Wardoc to clear space.
    Waypoints are loaded from waypoints_WardocTaming.json (use util_recordWaypoints.py to create it).
'''
...
def FindParagon():
    '''
    Finds the nearest untamed Wardoc within range to use as a kill target
    when follower slots are full.
    ...
    nearestDist = 9999
    for m in candidates:
        if m.Name.lower() == WARDOC_NAME.lower() and m.Serial not in alreadyTamedSerials:
...
def AttackParagonWithPets( paragon ):
    ...
    Misc.SendMessage( 'Kill target found — all kill: %s' % paragon.Name, colors[ 'cyan' ] )
...
def LoadWaypoints():
    if not os.path.exists( waypointsFile ):
        ...
        Misc.SendMessage( 'Run util_recordWaypoints.py first to create waypoints_WardocTaming.json.', colors[ 'red' ] )
        return None
...
def FindTameable():
    '''
    Finds the nearest untamed Wardoc within range.
    Notoriety == 1 (blue) means the mob is not yet owned.
    Returns the mobile or None.
    '''
    animalFilter = Mobiles.Filter()
    animalFilter.Enabled   = True
    ...
    # Exclude already-tamed (notoriety != 1 means owned) and session-tracked serials
    filteredList = GetEmptyMobileList( Mobiles )
    for m in candidates:
        if m.Name.lower() == WARDOC_NAME.lower() and m.Notoriety == 1 and m.Serial not in alreadyTamedSerials:
            filteredList.Add( m )
